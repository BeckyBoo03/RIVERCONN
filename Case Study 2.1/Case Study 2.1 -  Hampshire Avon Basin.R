...
shape_dams <- st_read("Case Study 2.1/Barriers/Hampshire_Avon.shp") %>% st_transform(roads, crs = st_crs(shape_basin))
...
# 1: DCI with symmetric passabilities
...
# 2: DCI with asymmetric passabilities
...
for (i in 1:length(d_index)) {
  ...
  plot_iter <- ggplot() +
    ...
    scale_color_viridis(name = "Ranking", direction = -1, limit = c(1, 9), breaks = c(1, 3, 5, 7, 9)) +
    theme_void() +
    guides(size = FALSE) +
