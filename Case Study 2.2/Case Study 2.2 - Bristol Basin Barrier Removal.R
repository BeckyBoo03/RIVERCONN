# Case Study 2.2 - Bristol Avon Basin - edited ####

# I have removed 50% of the barriers to see how that changes the results
...
shape_dams <- st_read("Case Study 2.2/Bristol_Avon_Basin/Barriers/New_Shapefile.shp")
...
ggplot() +
  coord_fixed() +
  ggspatial::layer_spatial(shape_river_simple, 
                           color = "gray25")+
  ggspatial::layer_spatial(dams_snapped_reduced, 
                           shape = 19) +
  theme_minimal() +
  theme(legend.position = "bottom")+
  ggspatial::annotation_north_arrow(location = "br")+
...
for (i in 1:length(d_index)) {
  ...
  plot_iter <- ggplot() +
    ...
    scale_color_viridis(name = "Ranking",direction = -1, limit= c(1, 75), breaks = c(1,25,75))+
    theme_void()+
    guides(size = FALSE) +
