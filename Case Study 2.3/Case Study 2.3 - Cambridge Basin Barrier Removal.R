...
# This is the code for the 50% reduction in the number of barriers in the basin
...
shape_dams <- st_read("Case Study 2.3/Cambridge/Export_Output_32.shp") %>% st_transform(roads, crs = st_crs(shape_basin))
...
ggplot() +
  ...
  scale_color_viridis(direction = -1, 
                      name= "Reach ID",
                      limits = c(1, 337), 
                      breaks=c(1,50,100,150,200,250,300,337)) +
...
ggplot() +
  ...
  scale_x_continuous(labels = function(x){
    ifelse(x < 0, paste0(abs(x), "\u00B0 W"), 
           ifelse(x > 0, paste0(x, "\u00B0 E"), "0"))
  }, limits = c(-1.2, 1), breaks = seq(-1.2, 1, by = 0.5))
