# Case Study 2.3 - Cambridge Basin ####
...
shape_basin <- st_read("Case Study 2.3/Cambridge/Export_Output_20.shp") 
shape_dams <- st_read("Case Study 2.3/Cambridge/Export_Output_22.shp") %>% st_transform(roads, crs = st_crs(shape_basin))
shape_river <- st_read("Case Study 2.3/Cambridge/Export_Output_25.shp") %>% st_transform(roads, crs = st_crs(shape_basin))
...
ggplot() +
  ...
  scale_color_viridis(direction = -1, name= "Reach ID",limits = c(1, 337),breaks=c(1,50,100,150,200,250,300,337)) +
  ...
  }, limits = c(-1.5, 1.5), breaks = seq(-1.5, 1.5, by = 0.5))
...
ggplot() +
  ...
  scale_x_continuous(labels = function(x){
    ifelse(x < 0, paste0(abs(x), "\u00B0 W"), 
           ifelse(x > 0, paste0(x, "\u00B0 E"), "0"))
  }, limits = c(-1.2, 1.2), breaks = seq(-1.2, 1, by = 0.5))+
...
# Save for checking
river_net_simplified %>% st_write("Case Study 4.2/diagnostics/river_net_simplified.shp", delete_dsn = TRUE)

# plot check - changed to orginal tutorial plot by Rebecca
ggplot() +
  coord_fixed() +
...
# 2.5 Adding addition attributes to the river shapefile ####
...
plot_final <- ggpubr::ggarrange(plotlist = plot_list, nrow = 2, ncol = 3, legend = "right")
plot_final
...
for (i in 1:length(d_index)) {
  ...
  plot_iter <- ggplot() +
    ...
    scale_color_viridis(name = "Ranking",direction = -1, limit= c(1, 141), breaks = c(1,50,100,141))+
    theme_void()+
    guides(size = FALSE) +
